...
from warningInfo import Warning
...
class Moderation(commands.Cog):
    ...
    @commands.command(name="GPT", help="Enables or disables the GPT moderation. Enter in 1 for enabled (default) or 0 for disabled", usage = "gpt 1")
    async def gpt(self, ctx, bool):
        db = GPTdb(bot = self.bot, guild = ctx.guild)
        if int(bool) == 1:
            bool = db.update_value(column="Enable", value=int(bool))
            await ctx.send("GPT moderation has been enabled!")
        elif int(bool) == 0:
            bool = db.update_value(column="Enable", value=int(bool))
            await ctx.send("GPT moderation has been disabled!")
        else:
            await ctx.send("Error! Please enter in 1 or 0 for your input!")
    ...
    @commands.command(name="Quit", help="Permantently shuts the bot down")
    async def quit(self, ctx):
        await ctx.send("Shutting down..... Head to https://mecsimcalc.com/app/0692014/discord_bot_creator to get a new bot and dont forget to reconfigure your settings!")
        sys.exit()
